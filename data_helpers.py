...
def load_data_and_labels(data):
    ...
    import pandas as pd
    df_ejemplo = pd.read_csv('data_ejemplo.csv', sep=';')
    L = df_ejemplo['catgy'].values.tolist()
    ...
    for lst in L:
        ...
        new_L.append(new_lst)


    labels = new_L.copy()
    x_text = df_ejemplo['text'].values.tolist()
    x_text = [clean_str(doc) for doc in x_text]
